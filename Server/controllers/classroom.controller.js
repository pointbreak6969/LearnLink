...
const deleteClassroom = asyncHandler(async (req, res)=> {
  ...
  const classroom = await Classroom.findById(classroomId)
    ...
    return res.status(200).json(new ApiResponse(201, {}, "classroom deleted successfully"))
})
...
const updateClassroom = asyncHandler(async (req, res) => {
  ...
  //check authorization
  // if (classroom.author !== req.user._id) {
  //     throw new ApiError(403, "Unauthorized: Only the author can update this classroom"); // Changed to 403 for "Forbidden"
  // }
  console.log(classroom.author !== req.user._id)
