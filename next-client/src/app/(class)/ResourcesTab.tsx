...
const ResourcesTab = ({ classroomId }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        if (debouncedTitle) {
          const response = await resourceService.getResourceByTitle(
            debouncedTitle
          );
          SetResources(response);
        } else {
          const response = await resourceService.getClassroomResources(
            classroomId
          );
          SetResources(response);
        }
            ...
            {resources.map((resource) => (
              ...
              >
                <span className="text-gray-800 flex items-center">
                  <FaRegFilePdf className="w-8 h-5 text-red-600 mr-2" />{" "}
                  {resource.title}
                </span>
